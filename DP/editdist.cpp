...
int main(int argc,char** argv){
	...
	for(int i=1;i<=len1;i++)
		dist[i][0]=dist[i-1][0]+del;	//some string to null
	for(int i=1;i<=len2;i++)
		dist[0][i]=dist[0][i-1]+add;
	for(int i=1;i<=len1;i++)
	{
		for(int j=1;j<=len2;j++){
			...
			dist[i][j]=min(min(dist[i-1][j-1]+replace, dist[i][j-1]+add), dist[i-1][j]+del);
			}
		}
	}
	...
	while(i!=0 || j!=0){
		...
		else if(dist[i][j]==dist[i-1][j-1] && argv[1][i-1]==argv[2][j-1]){
			i--;j--;
		}
		else if(dist[i][j]==dist[i][j-1]+add){
			cout<<"add "<<argv[2][j-1]<<endl;
			j--;
		}
		else if(dist[i][j]==dist[i-1][j]+del){
			cout<<"delete "<<argv[1][i-1]<<endl;
			i--;
		}
